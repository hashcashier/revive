...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=4cm,
  ...
  \draw[-, every node/.style={font=\sffamily\small}]
  	(A) edge node[near start] {500}
  			 node[near end] {600} (B)
  		edge node[right, very near start] {200}
  			 node[left, very near end] {300} (C)
  		edge node[left, near start] {250}
  			 node[left, near end] {320} (D)
  	(B) edge node[near start] {300}
  			 node[near end] {200} (C)
  		edge node[right, near start] {100}
  			 node[left, near end] {970} (D)
  	(C) edge node[near start] {370}
  		 	 node[near end] {1290} (D)
  	;
\end{tikzpicture}
\caption{Example set of nodes}
\end{figure}

\subsection{Cooperative specification of rebalancing requirements}
Participants specify the credits they wish to gain or lose in each channel

\subsection{Generation of rebalancing transaction set}
Linear program to solve constraints

\subsection{Atomic execution of rebalancing}
All payments must either execute or terminate
